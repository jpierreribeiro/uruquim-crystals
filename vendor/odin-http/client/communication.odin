...
parse_response :: proc(socket: Communication, allocator := context.allocator) -> (res: Response, err: Error) {
	...
	// URUQUIM CRYSTALS BRIDGE PATCH 3 (Phase 7.5-A2): free the scanner on an early
	// error return (a timed-out or reset read). Upstream only stores the scanner
	// on the success path (`res._body = scanner` at the end), leaking ~4 KiB per
	// failed request — a slow-drip DoS on the timeout path. On success `err` is
	// nil and ownership passes to res._body, so this defer does nothing.
	defer if err != nil {
